...
![Bayesian updating surface and slices](assets/bayesian-updating-surface.svg)

The figure makes the slice operation explicit in four linked panels: a sparse
3D wireframe, a top-down contour map, the horizontal fixed-\(\theta\) slice,
and the vertical fixed-\(x\) slice. The red horizontal line in the map becomes
the conditional density \(p(x\mid\theta_0)\) over observations. The blue
vertical line becomes the likelihood \(p(x_{\mathrm{obs}}\mid\theta)\) over
parameters.
...
The remaining panels show the prior, unnormalized posterior numerator,
normalized posterior, and evidence as a function of possible observations.
